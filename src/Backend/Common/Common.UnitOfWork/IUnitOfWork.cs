...
namespace Common.UnitOfWork
{
    ...
    public interface IUnitOfWork : IDisposable
    {
        ...
        /// </summary>
        /// <param name="ensureAutoHistory"><c>True</c> if save changes ensure auto record the change history.</param>
        /// <param name="userID"><c>LoginID</c> if save changes ensure auto record the change history.</param>
        /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous save operation. The task result contains the number of state entities written to database.</returns>
        Task<int> SaveChangesAsync(string userID, bool ensureAutoHistory = false);
        ...
        IList<T> ExecSQL<T>(string query);

        IList<T> ExecSQL<T>(string query, Dictionary<string, object> Params);
        ...
        Task<List<AuditoriaDto>> OnBeforeSaveChanges(string userId);
