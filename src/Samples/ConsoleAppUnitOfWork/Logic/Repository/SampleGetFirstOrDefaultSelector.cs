...
{
    public class SampleGetFirstOrDefaultSelector
    {
        ...
        public static void Executar()
        {
            try
            {
                ...
                var categories = _unitOfWork.GetRepository<Categories>().GetFirstOrDefault(selector: x => x.CategoryName);
                Console.ForegroundColor = ConsoleColor.Cyan;

                if (categories != null)
                {
                    Console.WriteLine("CategoryName : " + categories);
                }
            ...
            {
                Console.WriteLine($"GetFirstOrDefaultAsyncSelector encerrado inesperadamente Exceção: {ex.GetType().FullName} | " + $"Mensagem: {ex.Message}");
                throw;
            }
        }
