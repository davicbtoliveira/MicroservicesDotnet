...
public class SampleLongCount
{
    ...
    public static void Executar()
    {
        try
        {
            ...
            var contador = _unitOfWork.GetRepository<Categories>().LongCount();

            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.WriteLine("Total de registros encontrados: " + contador);
            Console.ReadKey();
            Console.WriteLine();
            Thread.Sleep(1000);
            Console.Clear();
        ...
        {
            Console.WriteLine($"LongCount encerrado inesperadamente Exceção: {ex.GetType().FullName} | " + $"Mensagem: {ex.Message}");
            throw;
        }
    }
