...
{
    public class SampleGetAllPredicateDisableTrackingOrderBy
    {
        public static void Executar()
        {
            try
            {
                ...
                var categories = _unitOfWork.GetRepository<Categories>().GetAll(predicate: x => x.CategoryID > 1 && x.CategoryID < 10,
            ...
            {
                Console.WriteLine($"SampleGetAllPredicateDisableTrackingOrderBy encerrado inesperadamente Exceção: {ex.GetType().FullName} | " + $"Mensagem: {ex.Message}");
                throw;
            }
        }
    }
