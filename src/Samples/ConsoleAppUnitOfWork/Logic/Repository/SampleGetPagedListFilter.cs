...
{
    public class SampleGetPagedListFilter
    {
        ...
        public static void Executar()
        {
            try
            {
                ...
                var filter = new PagedFilter<Categories>()
                  ...
                  .WithPageIndex(categoriesFilterDto.pageNo.GetValueOrDefault())
                  .WithPageSize(categoriesFilterDto.pageSize.GetValueOrDefault())
                  .WithOrderBy(x => Ordenar(categoriesFilterDto, x));

                var categories = _unitOfWork.GetRepository<Categories>().GetPagedList(filter);
            ...
            {
                Console.WriteLine($"GetPagedListFilter encerrado inesperadamente Exceção: {ex.GetType().FullName} | " + $"Mensagem: {ex.Message}");
                throw;
            }
        }
