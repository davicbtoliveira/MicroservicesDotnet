...
public class SampleSum
{
    ...
    public static void Executar()
    {
        try
        {
            ...
            var contador = _unitOfWork.GetRepository<Orders>().Sum(selector: x => (decimal)x.Freight);

            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.WriteLine("Soma entre registros Id calculada : " + contador);
            Console.ReadKey();
            Console.WriteLine();
            Thread.Sleep(1000);
            Console.Clear();
        ...
        {
            Console.WriteLine($"Sum encerrado inesperadamente Exceção: {ex.GetType().FullName} | " + $"Mensagem: {ex.Message}");
            throw;
        }
    }
