...
{
public class SampleGetAllSelector
{
    public static void Executar()
    {
        try
        {
            ...
            var categories = _unitOfWork.GetRepository<Categories>().GetAll(selector: x => x.CategoryName);

            Console.ForegroundColor = ConsoleColor.Cyan;

            foreach (var item in categories)
            {
                Console.WriteLine("CategoryName : " + item);
            }
        ...
        {
            Console.WriteLine($"SampleGetAllSelector encerrado inesperadamente Exceção: {ex.GetType().FullName} | " + $"Mensagem: {ex.Message}");
            throw;
        }
